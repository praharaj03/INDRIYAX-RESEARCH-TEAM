...
import { RiMailLine, RiPhoneLine, RiMapPinLine, RiTimeLine, RiContactsLine } from "react-icons/ri";

const contactItems = [
  { icon: RiMailLine, label: "Email", value: "contact@indriyax.com", href: "mailto:contact@indriyax.com" },
  { icon: RiPhoneLine, label: "Phone", value: "+91 98765 43210", href: "tel:+919876543210" },
  {
    icon: RiMapPinLine,
    label: "Address",
  ...
  { icon: RiTimeLine, label: "Clinic Hours", value: "Mon – Sat: 9:00 AM – 7:00 PM\nSunday: Closed", href: null },
];
          ...
          {contactItems.map(({ icon: Icon, label, value, href }, i) => (
            <AnimateIn key={label} delay={i * 0.08}>
              <div className="bg-dark-3 rounded-2xl border border-border p-5 hover:border-primary/30 card-glow transition-all">
                ...
                {href ? (
                  <a href={href} className="text-white text-sm hover:text-primary transition-colors whitespace-pre-line">
                    {value}
                  </a>
                ) : (
